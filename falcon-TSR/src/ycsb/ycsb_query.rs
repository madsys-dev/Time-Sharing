...
impl<'a> YcsbQuery<'a> {
    ...
    pub fn gen_request(&mut self, prop: &Properties) {
        ...
        for i in 0..prop.req_per_query {
            ...
            #[cfg(feature = "ycsb_d")]
            let mut key = self.counter.load(Ordering::Relaxed)
                - self.zipf(self.counter.load(Ordering::Relaxed), prop.zipf_theta);
            #[cfg(feature = "ycsb_e")]
            let mut key = self.zipf(
                self.counter.load(Ordering::Relaxed) - prop.workload.scan_len,
                prop.zipf_theta,
            );
            ...
            while v.contains(&key) {
                #[cfg(feature = "ycsb_d")]
                {
                    key = self.counter.load(Ordering::Relaxed)
                        - self.zipf(self.counter.load(Ordering::Relaxed), prop.zipf_theta);
                }
                #[cfg(feature = "ycsb_e")]
                {
            ...
            #[cfg(feature = "ycsb_f")]
            {
                if ro || op <= prop.workload.read_perc {
                    ...
                    self.requests[i * 2 + 1] =
                        YcsbRequest::new(Operation::Nop, key, "0".to_string(), column);
                } else {
                    self.requests[i * 2] =
                        YcsbRequest::new(Operation::Read, key, "0".to_string(), column);
                    self.requests[i * 2 + 1] =
            ...
            if ro || op <= prop.workload.read_perc {
                ...
                <= prop.workload.read_perc + prop.workload.write_perc + prop.workload.scan_perc
            {
                self.requests[i] = YcsbRequest::new_scan(
                    Operation::Scan,
                    key,
                    "0".to_string(),
                    column,
                    u64_rand_new(&mut self.rng, 1, prop.workload.scan_len),
                );
            } else {
                self.requests[i] = YcsbRequest::new(
                    Operation::Insert,
