...
impl<'a> YcsbTxn<'a> {
        ...
        #[cfg(not(feature = "ycsb_read_tuple"))]
        {
            match table.search_tuple_id(&IndexType::Int64(key)) {
                Ok(tid) => {
                    ...
                    if tid.get_address() <= 1000000 {
                        return false;
                    }
                    match self.txn.read_column(table, &tid, column) {
                        Ok(vec) => {
                            self.read_buf = vec.data[3];
                            // #[cfg(feature = "txn_clock")]
    ...
    pub fn insert_init(&mut self, table: &'a Table, value: &str) -> bool {
        self.txn.insert_direct(table, value);
        true
    }
    ...
    pub fn run_txn(&mut self, table: &'a Table, query: &YcsbQuery) -> bool {
        ...
        // println!("tsp begin time {:?}", a2.duration_since(a1).unwrap());
        for req in &query.requests {
            match req.op {
                Operation::Read => {
                    // let a1: SystemTime = SystemTime::now();
                    ...
                    // let a2: SystemTime = SystemTime::now();
                    // println!("tsp read time {:?}", a2.duration_since(a1).unwrap());
                }
                Operation::Update => {
                    // let a1: SystemTime = SystemTime::now();
                    ...
                    // let a2: SystemTime = SystemTime::now();
                    // println!("tsp update time {:?}", a2.duration_since(a1).unwrap());
                }
                Operation::Scan => {
        ...
        // println!("tsp commit time {:?}", a2.duration_since(a1).unwrap());
