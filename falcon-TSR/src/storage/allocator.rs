...
#[derive(Debug)]
pub struct LocalBufferAllocator {
    page_start: Address,
    offset: u32,
    max_tuple: u32,
    free_list: SegQueue<TupleId>,
...
impl LocalBufferAllocator {
    pub fn new(meta_page: Address, max_tuple: u32) -> Self {
        let mut allocator = LocalBufferAllocator {
            page_start: 0,
            offset: 0,
            max_tuple: max_tuple,
            free_list: SegQueue::new(),
        ...
        //         .push(storage.alloc_page().unwrap().page_start);
        // }
        allocator.data.reserve((PAGE_SIZE * 3) as usize);
        allocator.page_start = allocator.data.as_ptr() as Address;
        // println!("create page count {}", allocator.meta_page.len());
        allocator
    }
