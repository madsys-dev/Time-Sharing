...
impl Table {
        ...
        #[cfg(not(feature = "lock_index"))]
        {
            ...
            match (table_index, key) {
                ...
                (TableIndex::Int64R(index), IndexType::Int64(u)) => {
                    match index.get(u, &crossbeam_epoch::pin()) {
                        Some(v) => {
                            result = Ok(v.clone());
                        }
                        None => {
                ...
                _ => return Err(Error::Tuple(TupleError::IndexNotBuilt)),
            };
        }
        ...
        for (column_id, table_index) in self.index.iter() {
            ...
            match table_index {
                ...
                TableIndex::Int64(index) => {
                    match index.get(&u64::from_le_bytes(key.try_into().unwrap())) {
                        Some(v) => {
                            // println!("update index {} {} {}", v.get_address(), pool_address, new_address);
    ...
    pub fn index_insert_by_tuple_buffer(
        &self,
        tuple_id: &TupleId,
        tuple: &RwLockReadGuard<BufferDataVec>,
    ) -> Result {
        ...
        for (column_id, table_index) in self.index.iter() {
            ...
            #[cfg(not(feature = "lock_index"))]
            {
                match table_index {
                    TableIndex::Int64(index) => {
                        // println!("{}, {}", u64::from_le_bytes(key.try_into().unwrap()), tuple_id.get_address());
                        index.insert(
                            u64::from_le_bytes(key.try_into().unwrap()),
                            tuple_id.clone(),
                        );
                    ...
                    TableIndex::String(index) => {
                        let key = String::from(str::from_utf8(key).unwrap());
                        let key = key.trim_end_matches(char::from(0));
                        index.insert(key.clone(), key.len());
                    }
                    #[cfg(feature = "rust_map")]
                    TableIndex::Int64R(index) => {
                        index.insert(
    ...
    /// [key_lower, key_upper)
    pub fn range_tuple_id_on_index(
        &self,
        key_lower: &IndexType,
        key_upper: &IndexType,
        columns: usize,
        ...
        // let index = self.index.read().unwrap();
        // println!("key:{:?}, columns:{:?}, index:{:?}",key_lower,columns,self.index);
        let mut collect = Vec::<TupleId>::new();
        let result: Result<Vec<TupleId>>;

        let table_index = self.index.get(&columns).unwrap();
        // println!("key:{:?}, columns:{:?}, index:{:?}",key_lower,columns,table_index);
