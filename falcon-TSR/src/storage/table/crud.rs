...
impl Table {
    ...
    pub fn get_tuple(&self, tuple_id: &TupleId) -> Tuple {
        ...
        // assert!(tuple_id.get_address() & NVM_ADDR == NVM_ADDR);
        return Tuple::reload(self.get_address(&tuple_id));
    }
    pub fn allocate_tuple(&self, thread_id: usize) -> Result<TupleId> {
        ...
        {
            let mut allocator = self.allocator.get(thread_id).unwrap().write();
    ...
    pub fn update_tuple(
        ...
        #[cfg(feature = "buffer_pool")]
        {
            ...
            {
                let mut data: parking_lot::lock_api::RwLockWriteGuard<
                    '_,
                ...
                return Ok((0, pointer as u64 | POW_2_63));
            }
            ...
            #[cfg(feature = "zen")]
            {
                ...
                return Ok((0, new_pointer as u64 | POW_2_63));
            }
            #[cfg(not(feature = "zen"))]
            {
                return Ok((
                    old_data.len() as u64 + DELTA_DATA_OFFSET as u64 + U64_OFFSET,
                    new_pointer as u64 | POW_2_63,
                ));
            }
        }
    ...
    #[cfg(feature = "buffer_pool")]
    pub fn apply_tuple(&self, thread_id: usize, reset: bool, txn: &mut Transaction) {
        use std::time::SystemTime;

        let start_p: usize = self.pool_size / TRANSACTION_COUNT * thread_id;
        let mut pointer = self
            ...
            .clone();
        let end_p = self.pool_size / TRANSACTION_COUNT * (thread_id + 1);
        // println!("{} {}", start_p, pointer);
        #[cfg(feature = "undo_log")]
        for log in &txn.logs {
            let address = txn.txn_buffer.alloc();
            let mut tuple_dalta = TupleDelta::new(address, 0, log.tuple_id).unwrap();
            let log_data = log.data.as_slice();
            tuple_dalta.save(log_data);
            txn.txn_buffer.add_delta(tuple_dalta.len());
            tuple_dalta.clwb();
        }
        file::sfence();
        while pointer > start_p {
            ...
            let nvm_id = vec.nvm_id.load(Ordering::Relaxed);
            vec.clock.store(0, Ordering::Relaxed);
            // println!("pointer : {}", pointer);
            // println!("nvm_id : {}", nvm_id);
            // println!("data : {:?}", data.get_data(self.tuple_size));
            if nvm_id == 0 {
                // println!("read pointer : {}, start-end {} {}, table {}", pointer, start_p, end_p, self.id);
                // println!("nvm_id : {}", nvm_id);
                // println!("data : {:?}", data.get_data(self.tuple_size));
                continue;
            }
            assert!(nvm_id > 0);
            assert!(nvm_id < POW_2_63);
            // let a1 = SystemTime::now();

            Tuple::by_buffer(nvm_id, data.get_data(self.tuple_size));
            file::sfence();
            // let a2 = SystemTime::now();

            // println!("tsp tuple time {} {:?}", self.tuple_size, a2.duration_since(a1).unwrap());

            self.update_tuple_buffer_id_on_index(nvm_id, &data, (pointer) as u64 | POW_2_63)
                .unwrap_or(());
            if reset {
                // println!("write pointer : {}", pointer);
                // println!("nvm_id : {}", nvm_id);
                vec.nvm_id.store(0, Ordering::Relaxed);
                vec.insert.store(false, Ordering::Relaxed);
            }
        }
        if reset {
    ...
    pub fn apply_replica_tuple(
        ...
        while pointer > start_p {
            ...
            let key = data.get_data_by_column(range);
            match self
                .schema
                .get_column_type(self.primary_key.load(Ordering::Relaxed))
            {
                ColumnType::Int64 => {
                    let index_key = IndexType::Int64(u64::from_le_bytes(key.try_into().unwrap()));
                    if vec.insert.load(Ordering::Relaxed) {
                        let tuple_id = replica_table.allocate_tuple(thread_id).unwrap();
                        Tuple::by_buffer(tuple_id.get_address(), data.get_data(self.tuple_size));
                        replica_table.index_insert(index_key, &tuple_id).unwrap();
                    } else {
                        let tuple_id = replica_table.search_tuple_id(&index_key).unwrap();
                        assert!(tuple_id.get_address() & POW_2_63 == 0);
                        Tuple::by_buffer(tuple_id.get_address(), data.get_data(self.tuple_size));
                        // println!("sync end");
                    }
                ...
                ColumnType::String { len: _ } => {
                    let index_key = IndexType::String(String::from(str::from_utf8(key).unwrap()));
                    if vec.insert.load(Ordering::Relaxed) {
                        let tuple_id = replica_table.allocate_tuple(thread_id).unwrap();
                        Tuple::by_buffer(tuple_id.get_address(), data.get_data(self.tuple_size));
                        replica_table.index_insert(index_key, &tuple_id).unwrap();
            ...
            // replica_table.update_tuple_buffer_id_on_index(
            //     tuple_id.get_address(),
            //     &data,
            //     (pointer) as u64 | POW_2_63,
            // )
