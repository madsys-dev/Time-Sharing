...
impl Catalog {
    ...
    pub fn get_transaction_page_start(&self, thread_id: u64) -> u64 {
        let iter = unsafe { CATALOG_ADDRESS + thread_id * U64_OFFSET };
        unsafe { io::read(iter) }
    }
    ...
    pub fn update_ts(thread_id: u64, ts: u64) {
        let iter =
            unsafe { CATALOG_ADDRESS + TRANSACTION_TS_ADDRESS.start + thread_id * U64_OFFSET };
    ...
    pub fn init_catalog(root_addr: Address) {
        if CATALOG.get().is_none() {
            CATALOG.set(Catalog::new(root_addr)).unwrap();
        }
    }
    ...
    pub fn set_primary_key(&self, table_name: &str, key: usize) {
        ...
        drop(storage);
        //TODO Reload
        let mut new_table = Table::new(table.schema.clone(), table.version, table.id);
    ...
    pub fn set_pool_size(&self, table_name: &str, size: usize) {
        ...
        for key in table.get_index_key() {
            if table_name == "usertable" {
                ...
                new_table.add_index(*key).unwrap();
            } else if table_name.starts_with("ORDER-LINE") || table_name.starts_with("NEW-ORDER") {
                new_table.add_range_index(*key).unwrap();
            } else {
                new_table.add_index(*key).unwrap();
            }
...
impl Catalog {
    pub fn get_table3_names(name: &str) -> [String; 3] {
        let mut name0 = String::from(name);
        name0.push('0');

        let mut name1: String = String::from(name);
        name1.push('1');
    ...
    pub fn add_table3(&self, name: &str, schema: TableSchema) -> Result {
        let names = Catalog::get_table3_names(name);
        for name in names {
            self.add_table(name.as_str(), schema.clone()).unwrap();
        }
        Ok(())
    }
    pub fn set_primary_key3(&self, table_name: &str, key: usize) {
        let names = Catalog::get_table3_names(table_name);
        for name in names {
            self.set_primary_key(name.as_str(), key);
        }
    }
    pub fn set_range_primary_key3(&self, table_name: &str, key: usize) {
        let names = Catalog::get_table3_names(table_name);
        for name in names {
            self.set_range_primary_key(name.as_str(), key);
        }
    }
    pub fn add_index_by_name3(&self, table_name: &str, key: &str) {
        let names = Catalog::get_table3_names(table_name);
        for name in names {
            self.add_index_by_name(name.as_str(), key);
        }
    }
    pub fn add_range_index_by_name3(&self, table_name: &str, key: &str) {
        let names = Catalog::get_table3_names(table_name);
        for name in names {
            self.add_range_index_by_name(name.as_str(), key);
        }
    }
    #[cfg(feature = "buffer_pool")]
    pub fn set_pool_size3(&self, table_name: &str, size: usize) {
        let names = Catalog::get_table3_names(table_name);
        for name in names {
            self.set_pool_size(name.as_str(), size);
        }
    }
    pub fn get_table3(&self, table_name: &str) -> [Arc<Table>; 3] {
        let names = Catalog::get_table3_names(table_name);
        [
            self.table_index
                .read()
