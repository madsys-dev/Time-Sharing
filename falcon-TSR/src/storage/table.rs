...
use super::allocator::LocalBufferAllocator;
...
use crate::config::TRANSACTION_COUNT;
pub mod buffer;
pub mod crud;
pub mod index;

#[cfg(feature = "index_bplus_tree")]
pub type Index<T> = BplusTree<T, TupleId>;
#[cfg(feature = "rust_hash")]
pub type Index<T> = HashMap<T, TupleId>;
#[cfg(feature = "rust_dash")]
pub type Index<T> = DashMap<T, TupleId>;
#[cfg(feature = "dash")]
pub type Index<u64> = Dash<u64>;

#[cfg(feature = "rust_map")]
pub type RangeIndex<T> = BzTree<T, TupleId>;
#[cfg(feature = "nbtree")]
pub type RangeIndex<u64> = NBTree<u64>;

#[cfg(feature = "local_allocator")]
pub type TupleAllocator = LocalPageAllocator;
#[cfg(feature = "center_allocator")]
pub type TupleAllocator = DualPageAllocator;
// #[derive(Debug)]
pub enum TableIndex {
    Int64(Index<u64>),
    #[cfg(feature = "dash")]
    ...
    String(Index<String>),
    #[cfg(any(feature = "nbtree", feature = "rust_map"))]
...
impl TupleId {
    ...
    pub fn buffered(&self) -> bool {
        return self.get_address() >= POW_2_63;
    }
...
impl Table {
    ...
    pub fn new(schema: TableSchema, address: Address, id: u32) -> Self {
        ...
        let tuple_start = 0;
        let meta_page = PersistArray::new(address + U64_OFFSET);
        #[cfg(feature = "buffer_pool")]
        ...
        let mut table = Table {
            ...
            allocator: std::iter::from_fn(|| {
                Some(RwLock::new(TupleAllocator::new(
                    address + U64_OFFSET,
                    max_tuple as u32,
                )))
            })
            .take(std::cmp::max(THREAD_COUNT, TRANSACTION_COUNT) as usize)
            .collect(),
            #[cfg(feature = "center_allocator")]
    ...
    pub fn reload(schema: TableSchema, address: Address, id: u32) -> Self {
        ...
        let meta_page = PersistArray::reload(address + U64_OFFSET); // 第一个u64是version
        #[cfg(feature = "buffer_pool")]
        let mut buffer_pointer: Vec<RwLock<usize>> = Vec::new();
        #[cfg(feature = "buffer_pool")]
        for i in 0..TRANSACTION_COUNT {
    ...
    pub fn get_version(&self) -> u64 {
        unsafe { io::nt_load(self.version as *mut c_void, 0) }
    }
    pub fn set_version(&self, ver: u64) {
        unsafe { io::write(self.version, ver) };
    }
    pub fn add_version(&self, _: usize) {
        ...
        let ver = unsafe { &*(self.version as *const AtomicU64) };
        ver.fetch_add(1, Ordering::Relaxed);
        unsafe { io::clwb(self.version as *const u8) };
        // #[cfg(not(feature = "time_sharding"))]
        // {
        //     if thread_id == 0 {
        //         unsafe { io::write(self.version, self.get_version() + 1) };
        //     }
        // }
...
mod tests {
    ...
    fn test_add_get_update_apply() {
        ...
        drop(storage);
        let mut buffer: TransactionBuffer = TransactionBuffer::new(Catalog::global(), 0);
        ...
        let (vec1, pointer1) = table.buffer_replace(thread_id, 100);
        vec1.nvm_id.store(tuple_id.get_address(), Ordering::Relaxed);
        vec1.insert_tuple("257,4", &table.schema, ts);

        let tuple_id = TupleId::from_address(pointer1 as u64 | POW_2_63);

        // let t2 = Tuple::new(tuple_address, "258,4", &table.schema, ts).expect("");
        // println!("{:?}", t2.get_data_by_column(table.schema.get_column_offset(1)));
        let (vec2, pointer2) = table.buffer_replace(thread_id, 100);
        println!("insert {}", pointer2);
        vec2.nvm_id
            .store(tuple_id2.get_address(), Ordering::Relaxed);
        vec2.insert_tuple("258,4", &table.schema, ts);
        let tuple_id2 = TupleId::from_address(pointer2 as u64 | POW_2_63);
        table
            .index_insert(
                IndexType::Int64(257),
                &TupleId::from_address(pointer1 as u64 | POW_2_63),
            )
            .unwrap();
        table
            .index_insert_by_tuple_buffer(&tuple_id2, &vec2.data.read())
            .unwrap();
        ...
        println!("apply insert:");
        table.apply_tuple(thread_id, true, &mut txn);

        let tuple_id = table.search_tuple_id(&IndexType::Int64(257)).unwrap();
        let tuple_id2 = table.search_tuple_id(&IndexType::Int64(258)).unwrap();
        assert!(tuple_id.get_address() & POW_2_63 == 0);
        assert!(tuple_id2.get_address() & POW_2_63 == 0);

        println!("update tuple1 local:");
        table
            .fix_tuple(
                &tuple_id,
                new_address,
        ...
        let index_tuple_id: TupleId = table.search_tuple_id(&IndexType::Int64(257)).unwrap();
        let (tuple, _) = table.get_tuple_buffer(&index_tuple_id, thread_id, 100, 100);
        assert_eq!(
            tuple
                .data
                .read()
                .get_data_by_column(table.schema.get_column_offset(1))[0],
            0
        );

        let index_tuple_id: TupleId = table.search_tuple_id(&IndexType::Int64(258)).unwrap();
        let (tuple2, _) = table.get_tuple_buffer(&index_tuple_id, thread_id, 100, 100);
        assert_eq!(
            tuple2
                .data
                .read()
        ...
        table.apply_tuple(thread_id, true, &mut txn);
        let index_tuple_id: TupleId = table.search_tuple_id(&IndexType::Int64(257)).unwrap();
        let tuple = table.get_tuple(&index_tuple_id);
        assert_eq!(
            tuple.get_data_by_column(table.schema.get_column_offset(1))[0],
            0
        );

        let index_tuple_id: TupleId = table.search_tuple_id(&IndexType::Int64(258)).unwrap();
        let tuple2 = table.get_tuple(&index_tuple_id);
        // println!("{:?}", tuple2.get_data(table.tuple_size));
        assert_eq!(
            tuple2.get_data_by_column(table.schema.get_column_offset(1))[0],
            1
        );
    }
    #[cfg(feature = "buffer_pool")]
    #[test]
    fn test_replica_apply() {
        use crate::{
            storage::catalog::Catalog,
            transaction::{transaction::Transaction, transaction_buffer::TransactionBuffer},
        };
        ...
        let mut table = Table::new(schema.clone(), page_id.page_start, 0);
        let mut replica_table = Table::new(schema.clone(), page_id.page_start, 0);
        ...
        table.set_primary_key(0).unwrap();
        replica_table.set_primary_key(0).unwrap();
        // let _ = Tuple::new(tuple_address, "257,4", &table.schema, ts).expect("");
        let (vec1, pointer1) = table.buffer_replace(thread_id, 100);
        vec1.nvm_id.store(tuple_id.get_address(), Ordering::Relaxed);
        vec1.insert_tuple_data(
        ...
        let index_tuple_id = table.search_tuple_id(&IndexType::Int64(258)).unwrap();
        assert_eq!(index_tuple_id.get_address(), tuple_id2.get_address());
        println!("replica apply from table1 to table2");
        table.apply_replica_tuple(thread_id, &replica_table, false, &mut txn);
        println!("apply insert on table1:");
        table.apply_tuple(thread_id, true, &mut txn);

        let tuple_id = replica_table
            .search_tuple_id(&IndexType::Int64(257))
            .unwrap();
        let tuple_id2 = replica_table
            .search_tuple_id(&IndexType::Int64(258))
            .unwrap();
        assert!(tuple_id.get_address() & POW_2_63 == 0);
        assert!(tuple_id2.get_address() & POW_2_63 == 0);
        ...
        let index_tuple_id: TupleId = replica_table
            .search_tuple_id(&IndexType::Int64(257))
            .unwrap();
        let (tuple, _) = replica_table.get_tuple_buffer(&index_tuple_id, thread_id, 100, 100);
        assert_eq!(
            tuple
                .data
                .read()
                .get_data_by_column(schema.get_column_offset(1))[0],
            0
        );
        ...
        let index_tuple_id: TupleId = replica_table
            .search_tuple_id(&IndexType::Int64(258))
            .unwrap();
        let (tuple2, _) = replica_table.get_tuple_buffer(&index_tuple_id, thread_id, 100, 100);
        assert_eq!(
            tuple2
                .data
                .read()
        ...
        println!("replica apply from table2 to table1:");
        replica_table.apply_replica_tuple(thread_id, &table, false, &mut txn);
        println!("apply update on table2:");
        replica_table.apply_tuple(thread_id, true, &mut txn);

        println!("check table1:");
        let index_tuple_id: TupleId = table.search_tuple_id(&IndexType::Int64(257)).unwrap();
        let tuple = table.get_tuple(&index_tuple_id);
        assert_eq!(
            tuple.get_data_by_column(table.schema.get_column_offset(1))[0],
        ...
        println!("check table2:");
        let index_tuple_id: TupleId = replica_table
            .search_tuple_id(&IndexType::Int64(257))
            .unwrap();
        let tuple = replica_table.get_tuple(&index_tuple_id);
        assert_eq!(tuple.get_data_by_column(schema.get_column_offset(1))[0], 0);

        let index_tuple_id: TupleId = replica_table
            .search_tuple_id(&IndexType::Int64(258))
            .unwrap();
        let tuple2 = replica_table.get_tuple(&index_tuple_id);
        // println!("{:?}", tuple2.get_data(table.tuple_size));
        assert_eq!(tuple2.get_data_by_column(schema.get_column_offset(1))[0], 1);
    }
}
