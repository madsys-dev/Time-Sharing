...
impl<'a> Transaction<'a> {
    ...
    pub fn begin(&mut self) {
        ...
        // println!("tsp begin1 p time {:?}", a2.duration_since(a1).unwrap());
        #[cfg(feature = "read_ts")]
        {
    ...
    pub fn validation(&mut self) -> bool {
        for access in &self.read_set {
            if !access.tuple_id.buffered() {
                let tuple = access.table.get_tuple(&access.tuple_id);
                if !access.valid(&tuple.ts()) {
                    if tuple.ts().tid != 0 {
                        // read from cxl without modified
                        // println!("validate {:x} {:?} {:?} {}", access.tuple_id.get_address(), tuple.ts(), access.ts, self.ts.tid);
                        return false;
                    }
                }
                ...
                if !self.snapshot.access(ts, self.ts.tid, self.cur_min_txn) {
                    println!("validate {} {}", ts, self.ts.tid);
                    return false;
                }
            }
    ...
    pub fn commit(&mut self) -> bool {
        ...
        for ws in &mut self.write_set {
            ...
            if ws.is_insert {
                ...
                let delta_address = self.txn_buffer.alloc();
                let mut insert_delta =
                    TupleDelta::new(delta_address, 0, ws.tuple_id.get_address()).unwrap();
    ...
    pub fn insert_direct(&mut self, table: &'a Table, data: &str) -> TupleId {
    ...
    pub fn insert_direct3(&mut self, tables: &'a [Arc<Table>; 3], data: &str) -> [TupleId; 3] {
        let tuple_id0 = self.insert_direct(&tables[0], data);
        let tuple_id1 = self.insert_direct(&tables[1], data);
        let tuple_id2 = self.insert_direct(&tables[2], data);
        [tuple_id0, tuple_id1, tuple_id2]
    }
    ...
    #[cfg(feature = "buffer_pool")]
    pub fn insert_local(&mut self, table: &'a Table, data: &[u8]) -> TupleId {
        use std::sync::atomic::Ordering;

        let tuple_id = table.allocate_tuple(self.thread_id).unwrap();
        //println!("insert id {}", tuple_id.id);
        // let tuple_address = table.get_address(&tuple_id);
        let (vec1, pointer) = table.buffer_replace(self.thread_id, self.cur_min_txn);
        vec1.nvm_id.store(tuple_id.get_address(), Ordering::Relaxed);
        // println!("insert tuple {}", table.id);
        vec1.insert_tuple_data(data, &table.schema, self.ts);
        // println!("insert table, {}, {}", table.id, data);
        ...
        // let tuple = &Tuple::new(tuple_address, data, &table.schema, self.ts).unwrap();
        let local_tuple_id = TupleId::from_address(pointer as u64 | POW_2_63);

        table
            .index_insert_by_tuple_buffer(&local_tuple_id, &vec1.data.read())
            .unwrap();
        // tuple.clwb_len(table.schema.tuple_size() as u64);
        // debug!("txn {} insert tuple_id {}", self.ts, tuple_id.id);
    ...
    #[cfg(feature = "buffer_pool")]
    pub fn read_buffer(&mut self, table: &'a Table, tuple_id: &TupleId) -> Result<TupleVec> {
        #[cfg(feature = "clock")]
    ...
    pub fn read(&mut self, table: &'a Table, tuple_id: &TupleId) -> Result<TupleVec> {
        ...
        if tuple_id.get_address() & POW_2_63 > 0 {
            return self.read_buffer(table, &tuple_id);
        }
        // #[cfg(not(feature = "buffer_pool"))]
    ...
    pub fn delete(&mut self, table: &'a Table, tuple_id: &TupleId) -> Result<()> {
        #[cfg(feature = "hot_unflush")]
        let flush = self.flush_cache.access(tuple_id.get_address());
        self.write_set.push(WriteSetStruct::new(
            table,
    ...
    pub fn read_column(
        ...
        #[cfg(feature = "buffer_pool")]
        if tuple_id.get_address() & POW_2_63 > 0 {
            return self.read_buffer_column(table, &tuple_id, column_id);
        }
        // #[cfg(not(feature = "buffer_pool"))]
        let tuple_nvm = table.get_tuple(tuple_id);
        // tuple_nvm.apply_next(self.clog);
...
mod tests {
    ...
    fn test_insert_and_update_and_read() {
        ...
        let tuple1_id = transaction1.insert_direct(table, "233,666");
        println!("insert t1 233 666");
        let tuple2_id = transaction1.insert_direct(table, "666,233");
    ...
    fn test_range() {
        ...
        let tuple1_id = transaction1.insert_direct(table, "2,6");
        println!("insert t1 233 666");
        let tuple2_id = transaction1.insert_direct(table, "6,3");
        println!("insert t2 666 233");
        let tuple3_id = transaction1.insert_direct(table, "3,2");
        let t3_id = tuple3_id.clone();
        println!("insert t3 233 666");
        let tuple4_id = transaction1.insert_direct(table, "5,4");
        ...
        let r = table
            .range_tuple_id(&IndexType::Int64(1), &IndexType::Int64(6))
            .unwrap();
