...
#[cfg(feature = "ilog")]
const MANAGER_PAGE_SIZE: u64 = crate::config::YCSB_SIZE as u64 * 2 - 4 * U64_OFFSET;
#[cfg(feature = "ilog")]
const DRAM_PAGE_SIZE: u64 = PAGE_SIZE * 20 - 4 * U64_OFFSET;
#[cfg(not(feature = "ilog"))]
const MANAGER_PAGE_SIZE: u64 = PAGE_SIZE - 4 * U64_OFFSET;
const DATA_SIZE: u64 = crate::config::YCSB_SIZE as u64 + 256;
...
const STOP: u64 = EMPTY + U64_OFFSET;
...
impl TransactionBuffer {
    pub fn init(&mut self) {
        ...
        unsafe {
            ...
            *((self.address + STOP) as *mut u64) = 0;
        }
    ...
    pub fn is_stop(&self) -> bool {
        // println!("{} wait stop", self.address);
        return unsafe { *((self.address + STOP) as *const u64) == 1 };
    }
    pub fn stop(&self) {
        unsafe { *((self.address + STOP) as *mut u64) = 1 };
    }
    ...
    pub fn free(&mut self) {
        ...
        let mut storage = NVMTableStorage::global_mut();
        let page_start = storage.get_page_start_addr();
        storage.free_page_list(PageId::get_page_id(self.address, page_start))
    }
    pub fn reload(catalog: &Catalog, thread_id: u64) -> Self {
        ...
        let mut storage = NVMTableStorage::global_mut();
        for manager in &managers {
            let page = PageId::get_page_id(manager.address, storage.get_page_start_addr());
