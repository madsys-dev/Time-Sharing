...
pub fn run_new_order<'a>(
    ...
    txn.begin();
    txn.read_only = false;
    let warehouses = &table_list.warehouses[0];
    let districts = &table_list.districts[0];
    let customers = &table_list.customers[0];
    let orders = &table_list.orders[0];
    let new_orders = &table_list.new_orders[0];
    let items = &table_list.items[0];
    let stocks = &table_list.stocks[0];
    let order_lines = &table_list.order_lines[0];
    ...
    #[cfg(not(feature = "buffer_pool"))]
    {
        ...
        orders
            .index_insert_by_tuple(&TupleId::from_address(tuple._address()), &tuple)
            .unwrap();
        ...
        new_orders
            .index_insert(
                IndexType::Int64(order_key(wid, did, oid)),
                &TupleId::from_address(tuple._address()),
            )
            .unwrap();
        // println!("insert end");
    }
    #[cfg(feature = "buffer_pool")]
    {
        ...
        tuple.extend_from_slice(&order_key(wid, did, oid).to_le_bytes());
        tuple.extend_from_slice(&customer_key(wid, did, cid).to_le_bytes());
        tuple.extend_from_slice(&did.to_le_bytes());
        tuple.extend_from_slice(&wid.to_le_bytes());
        tuple.extend_from_slice(&entry_d.to_le_bytes());
        tuple.extend_from_slice(&0u64.to_le_bytes());
        tuple.extend_from_slice(&ol_cnt.to_le_bytes());
        tuple.extend_from_slice(&(all_local as u64).to_le_bytes());
        txn.insert_local(orders, tuple.as_slice());

        let mut tuple = Vec::new();
        tuple.extend_from_slice(&order_key(wid, did, oid).to_le_bytes());
        tuple.extend_from_slice(&did.to_le_bytes());
        tuple.extend_from_slice(&wid.to_le_bytes());
        txn.insert_local(new_orders, tuple.as_slice());
    }
    for olid in 0..ol_cnt {
        ...
        #[cfg(not(feature = "buffer_pool"))]
        {
            ...
            order_lines
                .index_insert(
                    IndexType::Int64(order_line_key(wid, did, oid, olid)),
                    &TupleId::from_address(tuple._address()),
                )
                .unwrap();
        }
        #[cfg(feature = "buffer_pool")]
        {
            ...
            tuple.extend_from_slice(&did.to_le_bytes());
            tuple.extend_from_slice(&wid.to_le_bytes());
            tuple.extend_from_slice(&order_line_key(wid, did, oid, olid).to_le_bytes());
            tuple.extend_from_slice(&ol_iid.to_le_bytes());
            if IS_FULL_SCHEMA {
                tuple.extend_from_slice(&0u64.to_le_bytes()); //OL_SUPPLY_WID
                tuple.extend_from_slice(&(Local::now().timestamp_nanos() as u64).to_le_bytes()); //OL_DELIVERY_ID
                tuple.extend_from_slice(&s_quantity.to_le_bytes());
                tuple.extend_from_slice(&_ol_amount.to_le_bytes());
                tuple.extend_from_slice(&0u64.to_le_bytes());
            }
            txn.insert_local(order_lines, tuple.as_slice());
        }
...
pub fn run_payment<'a>(
    ...
    let districts = &table_list.districts[0];
    let customers = &table_list.customers[0];
    let histories = &table_list.histories[0];
    let wid = payment.wid;
    let did = payment.did;
    ...
    let c_tid: TupleId;
    let cid: u64;
    let schema: &schema::TableSchema = &customers.schema;
    if by_last {
    ...
    #[cfg(feature = "buffer_pool")]
    {
        let mut tuple = Vec::new();
        tuple.extend_from_slice(&customer_key(wid, did, cid).to_le_bytes());
        tuple.extend_from_slice(&c_did.to_le_bytes());
        tuple.extend_from_slice(&c_wid.to_le_bytes());
        if IS_FULL_SCHEMA {
            tuple.extend_from_slice(&did.to_le_bytes()); //OL_SUPPLY_WID
            tuple.extend_from_slice(&wid.to_le_bytes()); //OL_DELIVERY_ID
            tuple.extend_from_slice(&0u64.to_le_bytes());
            tuple.extend_from_slice(&h_amount.to_le_bytes());
            tuple.extend_from_slice(&vec![0; 24]);
        } else {
            tuple.extend_from_slice(&0u64.to_le_bytes());
            tuple.extend_from_slice(&h_amount.to_le_bytes());
        }
        txn.insert_local(histories, tuple.as_slice());
    }
    if txn.commit() {
        return true;
    }
...
pub fn run_stock_level<'a>(
    ...
    match order_lines.range_tuple_id(
        &IndexType::Int64(min_orderline_key),
        &IndexType::Int64(max_orderline_key),
    ) {
        Ok(l) => {
            lines = l;
        }
    ...
    for olid in lines {
        match txn.read(order_lines, &olid) {
            Ok(row) => {
                ...
                let w_id =
                    u64::from_le_bytes(row.get_column_by_id(schema, column).try_into().unwrap());
                if w_id == wid {
                    let column: usize = schema.search_by_name("OL_QUANTITY").unwrap();
                    let quantity = u64::from_le_bytes(
                        row.get_column_by_id(schema, column).try_into().unwrap(),
                    );
                    ...
                    itemid2quantity
                        .insert(item_id, itemid2quantity.get(&item_id).unwrap() + quantity);
                }
            }
            _ => {
                txn.abort();
    ...
    for (item_id, _) in itemid2quantity.iter() {
        ...
        match stocks.search_tuple_id(&IndexType::Int64(stock_key(wid, *item_id))) {
            Ok(tid) => match txn.read_column(stocks, &tid, column) {
                Ok(row) => {
                    stock = row;
                }
                _ => {
        ...
        let s_quantity = u64::from_le_bytes(stock.data.as_slice().try_into().unwrap());
...
pub fn run_order_status<'a>(
    ...
    let mut oid: u64 = 0;
    // println!("1111");
    match orders.search_tuple_id_on_index(
        &IndexType::Int64(cid),
        schema.search_by_name("O_C_ID").unwrap(),
    ) {
        Ok(tid) => {
            //
            match txn.read(orders, &tid) {
                Ok(row) => {
                    oid = u64::from_le_bytes(
                        row.get_column_by_id(schema, schema.search_by_name("O_ID").unwrap())
                            .try_into()
                            .unwrap(),
                    );
...
pub fn run_deliver<'a>(
    ...
    for did in 0..DISTRICTS_PER_WAREHOUSE {
        loop {
            ...
            match new_orders.last_range_tuple_id(
                &IndexType::Int64(min_order_key),
                &IndexType::Int64(max_order_key),
            ) {
                Ok(tid) => {
                    no_tid = tid;
                    match txn.read(new_orders, &no_tid) {
                        Ok(row) => {
                            ...
                            oid = u64::from_le_bytes(
                                row.get_column_by_id(
                                    schema,
                                    schema.search_by_name("NO_O_ID").unwrap(),
                                )
                                .try_into()
                                .unwrap(),
                            );
            ...
            let cid =
                u64::from_le_bytes(order.get_column_by_id(schema, column).try_into().unwrap());
            ...
            for olid in lines {
                match txn.read(order_lines, &olid) {
                    Ok(row) => {
                        let ol_amount = f64::from_le_bytes(
                            row.get_column_by_id(
                                schema,
                                schema.search_by_name("OL_AMOUNT").unwrap(),
                            )
                            .try_into()
                            .unwrap(),
                        );
                ...
                match txn.update(
                    ...
                    &olid,
                    column,
                    &deliver.ol_delivery_d.to_le_bytes(),
                ) {
                    Ok(_) => {}
                    _ => {
                        txn.abort();
            ...
            #[cfg(not(feature = "append"))]
            {
                ...
                match txn.update(
                    ...
                    &c_tid,
                    column,
                    &(c_balance + ol_total).to_le_bytes(),
                ) {
                    Ok(_) => {}
                    _ => {
                        txn.abort();
            ...
            #[cfg(feature = "append")]
            {
                ...
                bytes.extend_from_slice(customer.get_column_by_id(schema, column));
