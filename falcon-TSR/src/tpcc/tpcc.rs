...
pub fn init_table_item(buffer: &mut TransactionBuffer) {
    ...
    let tables = &Catalog::global().get_table3("ITEM");
    ...
    for iid in 0..ITEMS {
        ...
        txn.insert_direct3(tables, &s);
        // index.add(item_key(iid), txn.insert_direct3(table, &s));
        s = String::new();
        s.reserve(40);
    }
    txn.commit();
...
pub fn init_table_warehouse(buffer: &mut TransactionBuffer, wid: u64) {
    ...
    let mut rng = &mut rng_mut;
    let mut txn = Transaction::new(buffer, false);
    let table = &Catalog::global().get_table3("WAREHOUSE");
    ...
    txn.insert_direct3(table, &s);
    // index.add(warehouse_key(wid), txn.insert_direct3(table, &s));
    txn.commit();
}
pub fn init_table_stock(buffer: &mut TransactionBuffer, wid: u64) {
    ...
    let mut txn = Transaction::new(buffer, false);
    let table = &Catalog::global().get_table3("STOCK");
    ...
    if IS_FULL_SCHEMA {
        for sid in 0..STOCKS_PER_WAREHOUSE {
            ...
            txn.insert_direct3(table, &s);
            // index.add(stock_key(wid, sid), txn.insert_direct3(table, &s));
            s = String::new();
            s.reserve(40);
        }
    } else {
...
pub fn init_table_district(buffer: &mut TransactionBuffer, wid: u64) {
    ...
    let mut txn = Transaction::new(buffer, false);
    let table = &Catalog::global().get_table3("DISTRICT");
    ...
    for did in 0..DISTRICTS_PER_WAREHOUSE {
        ...
        txn.insert_direct3(table, &s);
        // index.add(district_key(wid, did), txn.insert_direct3(table, &s));
        s = String::new();
        s.reserve(100);
    }
    txn.commit();
...
pub fn init_table_customer(buffer: &mut TransactionBuffer, wid: u64, did: u64) {
    ...
    let mut txn = Transaction::new(buffer, false);
    let table = &Catalog::global().get_table3("CUSTOMER");
    ...
    if IS_FULL_SCHEMA {
        for cid in 0..CUSTOMERS_PER_DISTRICT {
            ...
            txn.insert_direct3(table, &s);
            // index.add(customer_key(wid, did, cid), txn.insert_direct3(table, &s));
            s = String::new();
            s.reserve(100);
        }
    } else {
...
pub fn init_table_history(buffer: &mut TransactionBuffer, wid: u64, did: u64, cid: u64) {
    let mut txn = Transaction::new(buffer, false);
    let table = &Catalog::global().get_table3("HISTORY");
    let mut s = String::new();
    s.reserve(40);
    let mut rng_mut = rand::thread_rng();
    let mut rng = &mut rng_mut;
...
pub fn init_table_order(buffer: &mut TransactionBuffer, wid: u64, did: u64) {
    ...
    let mut txn = Transaction::new(buffer, false);
    let table = &Catalog::global().get_table3("ORDER");
    let table1 = &Catalog::global().get_table3("ORDER-LINE");
    let table2 = &Catalog::global().get_table3("NEW-ORDER");
    ...
    if IS_FULL_SCHEMA {
        for oid in 0..CUSTOMERS_PER_DISTRICT {
            ...
            append_u64(&mut s, 1);
            txn.insert_direct3(table, &s);
            // index.add(order_key(wid, did, oid), txn.insert_direct3(table, &s));
            s = String::new();
            for olid in 0..ol_cnt {
                ...
                txn.insert_direct3(table1, &s);
                // index1.add(order_line_key(wid, did, oid, olid), txn.insert_direct3(table1, &s));
                s = String::new();
                s.reserve(100);
            }
            if oid >= 2100 {
                ...
                append_u64(&mut s, did);
                append_u64(&mut s, wid);
                txn.insert_direct3(table2, &s);
                // index2.add(new_order_key(wid, did, oid), txn.insert_direct3(table2, &s));
                s = String::new();
                s.reserve(60);
            }
        }
        ...
        for oid in 0..CUSTOMERS_PER_DISTRICT {
            ...
            for olid in 0..ol_cnt {
                ...
                append_u64(&mut s, u64_rand(rng, 1, ITEMS) - 1);
                txn.insert_direct3(table1, &s);
                // table1.add_range_index(order_line_key(wid, did, oid, olid), txn.insert_direct3(table1, &s));
                s = String::new();
                s.reserve(60);
            }
            if oid >= 2100 {
