...
void CoroutineScheduler::PollLogCompletion() {
  for (auto it = pending_log_qps.begin(); it != pending_log_qps.end();) {
    ...
    if (poll_result == 0) {
        // RDMA_LOG(INFO) << pending_log_qps.size() << " " << wc.wr_id;
      it++;
      continue;
    }
    if (unlikely(wc.status != IBV_WC_SUCCESS)) {
      RDMA_LOG(EMPH) << "Bad completion status: " << t_id << " " << wc.status << " with error " << ibv_wc_status_str(wc.status) << ";@ node " << qp->idx_.node_id;
      if (wc.status != IBV_WC_RETRY_EXC_ERR) {
        RDMA_LOG(EMPH) << "completion status != IBV_WC_RETRY_EXC_ERR. abort()";
        abort();
      } else {
...
void CoroutineScheduler::PollCompletion() {
  // RDMA_LOG(INFO) << pending_log_qps.size() << " " << pending_qps.size();
    PollRegularCompletion();
// PollLogCompletion();  
  if (!LOCAL_LOCK)
    PollLogCompletion();
      // RDMA_LOG(INFO) << t_id << " " << pending_log_qps.size() << " " << pending_qps.size();
