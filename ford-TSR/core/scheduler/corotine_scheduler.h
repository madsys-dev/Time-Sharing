...
class CoroutineScheduler {
 public:
  ...
  bool RDMABatch2(coro_id_t coro_id, RCQP* qp, ibv_send_wr* send_sr, ibv_send_wr** bad_sr_addr, int doorbell_num);
  ...
  bool RDMAWrite2(coro_id_t coro_id, RCQP* qp, char* wt_data, uint64_t remote_offset, size_t size);
  ...
  int get_pending_log_size() {return pending_log_qps.size();}
 public:
...
ALWAYS_INLINE
bool CoroutineScheduler::RDMABatch2(coro_id_t coro_id, RCQP* qp, ibv_send_wr* send_sr, ibv_send_wr** bad_sr_addr, int doorbell_num) {
  send_sr[doorbell_num].wr_id = coro_id;
  auto rc = qp->post_batch(send_sr, bad_sr_addr);
  if (rc != SUCC) {
    RDMA_LOG(ERROR) << "client: post batch fail. rc=" << rc << ", tid = " << t_id << ", coroid = " << coro_id;
...
ALWAYS_INLINE
bool CoroutineScheduler::RDMAWrite2(coro_id_t coro_id, RCQP* qp, char* wt_data, uint64_t remote_offset, size_t size) {
  auto rc = qp->post_send(IBV_WR_RDMA_WRITE, wt_data, size, remote_offset, IBV_SEND_SIGNALED, coro_id);
  if (rc != SUCC) {
    RDMA_LOG(ERROR) << "client: post write fail. rc=" << rc << ", tid = " << t_id << ", coroid = " << coro_id;
    return false;
