...
class LogOffsetAllocator {
 public:
  LogOffsetAllocator(t_id_t tid, t_id_t num_thread) {
    auto per_thread_remote_log_buffer_size = (LOG_BUFFER_SIZE-1000) / num_thread;
    for (node_id_t i = 0; i < NUM_MEMORY_NODES; i++) {
      start_log_offsets[i] = tid * per_thread_remote_log_buffer_size + 100;
      end_log_offsets[i] = (tid + 1) * per_thread_remote_log_buffer_size + 100;
  ...
  offset_t GetTSROffset(node_id_t node_id) {
    return 0;
  }
