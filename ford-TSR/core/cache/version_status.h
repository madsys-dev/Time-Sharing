 ...
 public:
  ...
  void SetVersion(std::vector<DataSetItem>& read_write_set, tx_id_t tx_id) {
    ...
    for (auto& item : read_write_set) {
      ...
      for (int i = 0; i < SLOT_PER_BKT; i++) {
        if (!find_empty && (bkt->slots[i].key == 0 || bkt->slots[i].epoch != global_tsr_epoch)) {
          find_empty = true;
          target = i;
        } else if (bkt->slots[i].key == key && bkt->slots[i].epoch == global_tsr_epoch) {
      ...
      if (!find_match) {
        ...
        bkt->slots[target].epoch = global_tsr_epoch;
      }
    }
#elif ADJUST
    for (auto& item : read_write_set) {
  ...
  VersionStatus CheckVersion(std::vector<DataSetItem>& read_only_set, tx_id_t my_tx_id) {
    ...
    for (auto& item : read_only_set) {
      ...
      for (int i = 0; i < SLOT_PER_BKT; i++) {
        if (bkt->slots[i].key == key && bkt->slots[i].epoch == global_tsr_epoch) {
