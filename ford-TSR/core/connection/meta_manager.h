...
class MetaManager {
 public:
  ...
  ALWAYS_INLINE
  const MemoryAttr& GetRemoteTSRMR(const node_id_t node_id) const {
    auto mrsearch = remote_tsr_mrs.find(node_id);
    assert(mrsearch != remote_tsr_mrs.end());
    return mrsearch->second;
  }
 private:
  ...
  std::unordered_map<node_id_t, MemoryAttr> remote_tsr_mrs;
