...
MetaManager::MetaManager() {
  ...
  std::string config_filepath = "/mnt/mw.json";
  auto json_config = JsonConfig::load_file(config_filepath);
  auto local_node = json_config.get("local_compute_node");
  mechine_id = local_machine_id = (node_id_t)local_node.get("machine_id").get_int64();
...
void MetaManager::GetMRMeta(const RemoteNode& node) {
  ...
  MemoryAttr remote_hash_mr{}, remote_log_mr{}, remote_tsr_mr{};
  ...
  // while (QP::get_remote_mr(node.ip, node.port, SERVER_TSR_BUFF_ID, &remote_tsr_mr) != SUCC) {
  //   usleep(2000);
  // }
  remote_log_mrs[node.node_id] = remote_log_mr;
  remote_hash_mrs[node.node_id] = remote_hash_mr;
  // remote_tsr_mrs[node.node_id] = remote_tsr_mr;
}