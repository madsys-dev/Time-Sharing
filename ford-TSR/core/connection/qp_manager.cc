...
void QPManager::BuildQPConnection(MetaManager* meta_man) {
  for (const auto& remote_node : meta_man->remote_nodes) {
    ...
    // MemoryAttr remote_tsr_mr = meta_man->GetRemoteTSRMR(remote_node.node_id);
    ...
    // RCQP* tsr_qp = meta_man->global_rdma_ctrl->create_rc_qp(create_rc_idx(remote_node.node_id, (int)global_tid * 3 + 2),
    //                                                         meta_man->opened_rnic,
    //                                                         &local_mr);
    ...
    // do {
    //   rc = tsr_qp->connect(remote_node.ip, remote_node.port);
    //   if (rc == SUCC) {
    //     tsr_qp->bind_remote_mr(remote_tsr_mr);  // Bind the log mr as the default remote mr for convenient parameter passing
    //     tsr_qps[remote_node.node_id] = tsr_qp;
    //     // RDMA_LOG(INFO) << "Thread " << global_tid << ": Log QP connected! with remote node: " << remote_node.node_id << " ip: " << remote_node.ip;
    //   }
    //   usleep(2000);
    // } while (rc != SUCC);
