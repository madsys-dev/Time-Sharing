...
bool DTX::TxExe(coro_yield_t& yield, bool fail_abort) {
  ...
  if (global_meta_man->txn_system == DTX_SYS::FORD || global_meta_man->txn_system == DTX_SYS::LOCAL) {
    // Run our system
  ...
  } else if (global_meta_man->txn_system == DTX_SYS::FaRM || global_meta_man->txn_system == DTX_SYS::DrTMH ) {
    if (read_write_set.empty()) {
