...
bool InvisibleWriteBatch::SendReqs2(CoroutineScheduler* coro_sched, RCQP* qp, coro_id_t coro_id, int use_cas) {
  if (use_cas) {
    // sr[0] must be an atomic operation
    sr[0].wr.atomic.remote_addr += qp->remote_mr_.buf;
    sr[0].wr.atomic.rkey = qp->remote_mr_.key;
  ...
  if (!coro_sched->RDMABatch2(coro_id, qp, &(sr[0]), &bad_sr, 1)) return false;
  return true;
}
